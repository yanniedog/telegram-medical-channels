#!/usr/bin/env python3
"""Discover psych/pharm/dental/ophth/ent book channels for swarm JSON."""
...
BOOKS_RE = re.compile(r"books?|pdfs?|ebooks?|epubs?|library|textbook", re.I)

HEADERS = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
MIN_SUBS = 2000
OUT = Path("data/agent_swarm/swarm_psych_pharm_dental.json")
...
SEED_HANDLES = [
    "wafaobgyn", "ob_gyn_books", "pediatric_pdfs", "pediatrics_books",
...
def specialty_tags(username: str, title: str = "", description: str = "") -> list[str]:
    ...
    if re.search(r"paediatr|pediatr|child health|peds?\b|paeds?\b|child medic", blob, re.I):
    ...
    with httpx.Client(headers=HEADERS, follow_redirects=True) as client:
        ...
        for i, uname in enumerate(sorted(seen)):
            ...
            if not has_books_pdfs_signal(uname, title, desc):
                continue
            tags = meta.get("agent_specialty_tags") or specialty_tags(uname, title, desc)
            if not tags and not matches_specialty(uname, title, desc):
                continue
            if not tags:
                tags = specialty_tags(uname, "", "")
            if not tags:
                continue
            results.append({
                "username": uname,
